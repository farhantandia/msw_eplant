...
## MSW ePlant Mobile Application — Version 4.7
**PT Makmur Sejahtera Wisesa (MSW) — Adaro Group**
**2×30 MW CFPP + 868 kWp Solar PV Plant**
...
| v4.4 | Sep 2026 | Overhaul Penuh Modul Solar PV: Integrasi backend OpenAPI Huawei FusionSolar (`FusionSolarService`) live streaming & offline resilience, Arsitektur Dual-Plant (PLTS MSW 400 kWp, 8 Inverter + PLTS Kelanis 468 kWp, 4 Inverter = 868 kWp), Dedicated `SolarDetailPage`, Landscape Full-Screen Visualizer (`SolarLandscapeTrendPage`), Numeric Data Table modal (`SolarNumericTrendSheet`), Inverter Card Grid real-time dengan status LED, efisiensi & detail per unit (`InverterDetailPage`), kalkulasi dinamis Irradiance, Performance Ratio, CO₂ avoided, WhatsApp report generator, serta slide carousel plant status di homepage tanpa nested scroll. |
| v4.5 | Sep 2026 | Implementasi Penuh Plant Overview Detail Page & Integrasi Tabel Firebase RTDB `excel_data/overview` (14 parameter umum: TMGCR, BMCR, Boiler Efficiency, EAF, CF, Load PLN, Load AI, Total Load, House Load), Jaminan Zero Hardcoded Data (100% dinamis dari Firebase RTDB dengan fallback anggun ke `table1`/`table2`), Diagram Alir Energi Animasi SCADA (`PlantEnergyFlowWidget`) dengan Busbar 20kV / 70kV, Evaluasi Kondisi Operasional Unit 1 & 2 (Normal, Low Load, House Load, Shutdown), CEMS 4 Parameter Utama (SO₂, NOₓ, Partikulat, CO) dengan Baku Mutu Permen LHK No. P.15/2019, serta grafik tren interaktif 4 parameter (`fl_chart`). Semua parameter dan node diagram alir interaktif dapat diklik untuk membuka `ChartPage`. Fitur Share Dashboard Eksekutif berupa gambar PNG beresolusi tinggi (2.5x pixel ratio) untuk Plant Overview dan Solar PV via `DashboardShareService`. |
| v4.6 | Sep 2026 | Penyempurnaan Navigasi CFPP, Mesin Sinkronisasi OpenAPI & Minimalisasi Indikator: Shortcut menu homepage "Plant" di-rename menjadi "CFPP" dengan direct routing ke `PlantOverviewDetailPage`. Penjadwalan auto-sync periodik 30 menit (jam bulat & setengah jam: 09:00, 09:30, dst.) + sinkronisasi manual ke interval terdekat. Perbaikan kalkulasi produksi harian akurat: prioritas `day_cap` live dari OpenAPI (384.3+ kWh MSW) dan filter midnight (00:00:00) agar data kemarin tidak tercampur. Minimalisasi indikator loading: tepat satu bar linear tipis (2.0px) di bawah AppBar tanpa circular spinner dan tanpa teks tambahan di seluruh halaman aplikasi. Pendokumentasian lengkap fitur live dan roadmap teknis (Production D365 ERP Gateway, KLHK SISPEK, FCM Alerts). |
| **v4.7** | **Sep 2026** | **Pembersihan Modul ESG, Quad-Metrik Pembangkit di Homepage, Proteksi Rate-Limit 407 2x & Integrasi Weather Page**: Penghapusan total data dan komponen UI ESG (reduksi emisi CO₂, penghematan batubara, pohon yang ditanam) dari seluruh modul Solar PV untuk memfokuskan antarmuka pada telemetri teknis murni. Pembaruan 4 metrik pada kartu Solar PV di homepage menjadi metrik operasional per plant (MSW Irr, MSW PR, Kelanis Irr, Kelanis PR). Penerapan proteksi anti-blocking rate-limit 407 2x pada endpoint `/getDevList` (berhenti langsung pada percobaan ke-2 tanpa delay tambahan dan abort loop stasiun jika daftar inverter kosong). Integrasi shortcut 1-tap ke `WeatherPage` langsung dari widget alir energi surya. |
...
| **Solar PV Display** | **✅** | **Terintegrasi Huawei FusionSolar OpenAPI (Dual-plant 868 kWp: MSW 400 kWp + Kelanis 468 kWp)** |
...
| **CEMS 4 Parameter Utama (SO₂, NOₓ, Partikulat, CO)** | **✅** | **Nilai real-time dari RTDB (`cems1`/`cems2`) dengan Baku Mutu Permen LHK No. P.15/2019 + Tap to open `ChartPage`** |
...
| CEMS Threshold Baku Mutu | ✅ | Baku Mutu terintegrasi di `cems_threshold_service.dart` & `PlantOverviewSnapshot` |
| Pull-to-Refresh | ✅ | Di homepage & detail pages |
| Unit 1 & 2 Sensor Grid | ✅ | Color-coded + alarm |
| Trip/Shutdown Banner | ✅ | Load < 2 MW |
| **Solar PV Dedicated Page** | **✅** | **Terimplementasi Penuh: `SolarDetailPage`, Landscape Chart, Numeric Sheet, Inverter Grid & Detail, Share Dashboard Image (PNG)** |
| **Plant Overview Detail Page** | **✅** | **Terimplementasi Penuh: `PlantOverviewDetailPage`, Hero Balance Card, Kondisi Unit 1 & 2, Capability Matrix, CEMS Strip, Trend Chart. Semua metrik dapat membuka `ChartPage`** |
| **Diagram Alir Energi Animasi (Busbar 20kV / 70kV)** | **✅** | **Terimplementasi Penuh: `PlantEnergyFlowWidget` (Unit 1 & 2 -> Busbar 20kV/70kV -> PLN, AI, House Load) dengan modal inspeksi teknis & tombol tren historis** |
| **Share Dashboard Image (PNG)** | **✅** | **Terimplementasi Penuh: `DashboardShareService` mengekspor tampilan dashboard utuh beresolusi tinggi ke PNG via native share (WhatsApp, Email, Telegram, dll.)** |
| **Tabel RTDB 'excel_data/overview'** | **✅** | **Terintegrasi: 14 parameter umum dengan zero hardcoded data & graceful fallback ke `table1`/`table2`** |
...
| **CFPP Homepage Shortcut** | **✅** | **MenuGrid "Plant" di-rename "CFPP" & direct navigation ke `PlantOverviewDetailPage`** |
| **FusionSolar 30-Min Cron Sync Engine** | **✅** | **Auto-sync periodik setiap 30 menit (jam bulat & setengah) + sinkronisasi manual ke interval terdekat** |
| **Kalkulasi Produksi Solar Akurat** | **✅** | **Prioritas live `day_cap` dari OpenAPI & filter midnight (00:00:00) cegah akumulasi nilai kemarin** |
| **Indikator Loading Minimalis** | **✅** | **Satu linear progress bar 2.0px di bawah AppBar/header, tanpa circular loading & tanpa teks ekstra** |
| **Pembersihan Modul ESG & Fokus Telemetri** | **✅** | **Penghapusan total data & card ESG di Solar Detail & Inverter Detail untuk fokus pada telemetri teknis murni** |
| **Quad-Metrik Pembangkit Homepage** | **✅** | **4 kolom metrik Solar PV di homepage: MSW Irr, MSW PR, Kelanis Irr, Kelanis PR** |
| **Proteksi Rate-Limit 407 2x (/getDevList)** | **✅** | **Penghentian langsung retry setelah 2x failCode 407 pada `/getDevList` tanpa delay tambahan dan abort loop stasiun** |
| **Integrasi Weather Page dari Solar Flow** | **✅** | **Tombol 1-klik di header SolarEnergyFlowWidget yang terhubung langsung ke `WeatherPage`** |
| **Production D365 Gateway & VPN** | **🔜** | **Roadmap migrasi dari simulator service ke live Adaro APIM / VPN gateway & Entra ID token rotation** |
| **KLHK SISPEK CEMS Integration** | **🔜** | **Roadmap integrasi API transmisi emisi otomatis ke server SISPEK Kementerian LHK** |
...
Internet:         http ^1.2.1 ✅
Solar PV Backend: Huawei FusionSolar OpenAPI (FusionSolarService) ✅
SCADA Visuals:    PlantEnergyFlowWidget (CustomPainter Busbar 20kV / 70kV) ✅
URL Launcher:     url_launcher ^6.3.1 ✅
QR/Barcode Code:  mobile_scanner ^6.0.11 ✅ (aktif di modul Warehouse D365)
WhatsApp Share:   share_plus ^10.1.4 ✅ (aktif di laporan Solar PV & Logsheet)
ERP Integration:  Microsoft Dynamics 365 SCM REST API (D365Service) ✅
Hashing:          crypto ^3.0.6 🔜 (terpasang, target SHA-256+salt untuk auth Firestore)
...
```
/excel_data/
├── /overview/ → Ringkasan Umum Pembangkit & Rating Operasional
│   Array[]: [DATETIME, UNIT 1 TMGCR, UNIT 2 TMGCR, UNIT 1 BMCR, UNIT 2 BMCR,
│             UNIT 1 BOILER EFFICIENCY, UNIT 2 BOILER EFFICIENCY,
│             UNIT 1 EAF, UNIT 2 EAF, UNIT 1 CAPACITY FACTOR, UNIT 2 CAPACITY FACTOR,
│             LOAD TO PLN, LOAD TO AI, TOTAL LOAD, TOTAL HOUSE LOAD]
├── /table1/   → Boiler Unit 1 [DATETIME, UNIT 1 LOAD, ...]
├── /table2/   → Boiler Unit 2 [DATETIME, UNIT 2 LOAD, ...]
├── /cems1/    → CEMS Unit 1 [SO2, NOX, PARTICULATE, HG, CO, ...]
├── /cems2/    → CEMS Unit 2
└── /nphr/     → [DATETIME, NPHR UNIT 1, NPHR UNIT 2]
```

#### ✅ Huawei FusionSolar OpenAPI (Aktif — Solar PV Backend)

```
Huawei FusionSolar Cloud API:
├── Plant MSW (400 kWp)     → 8 Inverter (4 cluster: Atap WH, Atap Mess, Ground-mount, Canopy)
├── Plant Kelanis (468 kWp) → 4 Inverter
└── Metrik Real-time       → Daya Aktif (kW), Radiasi/Irradiance (kWh/m²), Performance Ratio (%),
                              Yield Harian/Kumulatif (kWh), Emisi CO₂ Avoided (Ton)
```
...
#### 4.3.1 Plant Monitoring & Plant Overview Detail Page ✅

Akses: Tap kartu status pembangkit di Homepage (`TotalGenerationCard`) atau menu Operation → Plant Overview.

##### A. Arsitektur Data `excel_data/overview` & Jaminan Zero Hardcoded Data
Data operasional umum pembangkit yang sebelumnya terduplikasi atau terpencar di `table1` dan `table2` telah dipusatkan ke node Firebase Realtime Database: `/excel_data/overview`.
- **14 Parameter Utama `excel_data/overview`**:
  1. `UNIT 1 TMGCR` (MW) — Rating Turbine Maximum Generator Continuous Rating Unit 1 (kapasitas terpasang 30 MW)
  2. `UNIT 2 TMGCR` (MW) — Rating Turbine Maximum Generator Continuous Rating Unit 2 (kapasitas terpasang 30 MW)
  3. `UNIT 1 BMCR` (t/h) — Boiler Maximum Continuous Rating Unit 1 (kapasitas steam terpasang 130 t/h)
  4. `UNIT 2 BMCR` (t/h) — Boiler Maximum Continuous Rating Unit 2 (kapasitas steam terpasang 130 t/h)
  5. `UNIT 1 BOILER EFFICIENCY` (%) — Efisiensi termal pembakaran Boiler Unit 1
  6. `UNIT 2 BOILER EFFICIENCY` (%) — Efisiensi termal pembakaran Boiler Unit 2
  7. `UNIT 1 EAF` (%) — Equivalent Availability Factor Unit 1 (keandalan unit)
  8. `UNIT 2 EAF` (%) — Equivalent Availability Factor Unit 2 (keandalan unit)
  9. `UNIT 1 CAPACITY FACTOR` (%) — Faktor kapasitas operasional Unit 1
  10. `UNIT 2 CAPACITY FACTOR` (%) — Faktor kapasitas operasional Unit 2
  11. `LOAD TO PLN` (MW) — Daya aktif tersalur ke jaringan transmisi interkoneksi PLN 150 kV
  12. `LOAD TO AI` (MW) — Daya aktif tersalur ke captive customer PT Adaro Indonesia 6.6 kV
  13. `TOTAL LOAD` (MW) — Total beban pembangkitan kotor gabungan (Gross Generation)
  14. `TOTAL HOUSE LOAD` (MW) — Konsumsi pemakaian sendiri peralatan auxiliary pembangkit
- **Jaminan Zero Hardcoded Data (100% Dynamic Telemetry)**:
  - Seluruh angka beban, rating kapasitas, efisiensi, dan ketersediaan ditarik secara dinamis dari stream RTDB (`snapshot.overview`, `snapshot.table1`, `snapshot.table2`, `snapshot.cems1`, `snapshot.cems2`, `snapshot.nphr`).
  - Bila suatu parameter belum terisi pada Firebase RTDB, sistem menyajikan format placeholder anggun bergaris `" — "` (tidak pernah merekayasa atau menampilkan angka statis/palsu).
  - *Graceful Fallback*: Jika node `overview` sedang kosong atau dalam inisialisasi, aplikasi secara otomatis fallback membaca data beban dari `table1` dan `table2`.

##### B. Diagram Alir Energi Animasi SCADA (`PlantEnergyFlowWidget`)
Diagram garis tunggal (*Single-Line SCADA Diagram*) kustom menggunakan Flutter `CustomPainter` yang menyajikan aliran energi dinamis dari turbogenerator hingga beban akhir:
- **Arsitektur Busbar Listrik & Tingkat Tegangan**:
  - **Generator Unit 1 & Unit 2**: Pembangkitan daya pada tegangan nominal generator **20 kV**.
  - **BUSBAR 20kV / 70kV**: Busbar interkoneksi sentral yang mengumpulkan daya dari turbogenerator 20 kV dan mendistribusikannya via trafo interkoneksi/step-up ke grid eksternal maupun auxiliary.
  - **PLN 150 kV**: Evakuasi daya ke transmisi interkoneksi PLN Kalselteng via Switchyard / Trafo Step-Up 70kV/150kV.
  - **PT Adaro Indonesia (AI) 6.6 kV**: Distribusi daya industri untuk operasional pertambangan Adaro via Feeder 6.6 kV.
  - **House Load 6.6 kV**: Distribusi pemakaian sendiri internal untuk auxiliary PLTU (BFP, ID Fan, FD Fan, ESP, CWP).
- **Animasi Partikel Pulsa Listrik**:
  - Animasi partikel mengalir (*particle pulses*) secara berulang sepanjang jalur konektor konduktor listrik untuk memvisualisasikan transmisi daya aktif.
  - Kecepatan dan visibilitas partikel disesuaikan dengan kondisi daya nyata (partikel berhenti mengalir jika unit dalam status Trip/0 MW).
- **Interaktivitas Node**: Setiap node pembangkit (Unit 1, Unit 2, Busbar 20kV/70kV, PLN, AI, House Load) dapat ditap untuk membuka modal technical spec dialog yang menampilkan tegangan, daya aktual, rating terpasang, dan status operasional.

##### C. Status & Kondisi Operasional Unit 1 & 2
Kondisi operasional unit dievaluasi secara cerdas berbasis beban aktual real-time:
- **Normal Running** (Beban $\ge$ 12.0 MW): Badge status hijau menyala dengan pulsing glow dot.
- **Low Load** (4.0 MW $\le$ Beban < 12.0 MW): Badge status kuning/amber mengingatkan kondisi operasi beban rendah.
- **House Load** (0.0 MW < Beban < 4.0 MW): Badge status ungu mengindikasikan unit beroperasi hanya menyuplai beban internal (*islanding mode*).
- **Shutdown / Trip** (Beban $\le$ 0.0 MW): Badge status merah dengan aksen bahaya dan peringatan unit tidak beroperasi.
- Menampilkan persentase pembebanan dinamis terhadap rating TMGCR (misal: $28.5 \text{ MW} / 30.0 \text{ MW} = 95.0\%$).

##### D. 4 Parameter CEMS Utama & Kepatuhan Regulasi
Menampilkan strip pemantauan 4 parameter CEMS utama di cerobong emisi:
1. **SO₂** (Sulfur Dioksida) — Baku Mutu: 550 mg/Nm³ (Permen LHK No. P.15/2019)
2. **NOₓ** (Nitrogen Oksida) — Baku Mutu: 550 mg/Nm³
3. **Partikulat (PM)** — Baku Mutu: 50 mg/Nm³
4. **CO** (Karbon Monoksida) — Baku Mutu: Nilai pemantauan operasional pembakaran
- Disertai badge status kepatuhan (*Compliant* hijau / *Exceed* merah) dan tautan langsung 1-klik menuju halaman detail `CemsDetailPage`.

##### E. Multi-Parameter Trend Chart Interaktif (`fl_chart`)
Grafik tren garis interaktif dengan pilihan switcher chip 4 metrik:
1. **Beban & House Load**: Perbandingan tren Total Beban (MW) vs Total House Load (MW).
2. **PLN vs AI**: Perbandingan tren alokasi daya ke Grid PLN vs PT Adaro Indonesia.
3. **Boiler Efficiency**: Perbandingan tren efisiensi termal Boiler Unit 1 vs Boiler Unit 2.
4. **NPHR Heat Rate**: Perbandingan efisiensi pemakaian panas Net Plant Heat Rate Unit 1 vs Unit 2 (kCal/kWh).
- Tooltip interaktif saat disentuh, gradient fills di bawah kurva, dan layout responsif tanpa overflow di seluruh resolusi layar (320px–412px dan font scale 1.0–1.5).
...
#### 4.3.3 Solar PV Dedicated Page & OpenAPI Huawei FusionSolar ✅

Akses: Tap kartu status Solar PV di Homepage (`SolarPvCard`) atau menu Operation → Solar PV.

##### A. Arsitektur Dual-Plant 868 kWp
Sistem Solar PV memonitor dua lokasi pembangkitan listrik tenaga surya terpisah di lingkungan PT MSW:
1. **PLTS MSW (400 kWp)**:
   - Terdiri dari 8 Inverter Huawei (Cluster: Atap Warehouse, Atap Mess Karyawan, Ground-mount, dan Canopy Parkir).
   - Rating terpasang: 400 kWp.
2. **PLTS Kelanis (468 kWp)**:
   - Terdiri dari 4 Inverter Huawei kapasitas besar di area Pelabuhan Terminal Kelanis.
   - Rating terpasang: 468 kWp.
- **Total Kapasitas Terpasang**: **868 kWp** (12 unit inverter total).

##### B. Integrasi Backend OpenAPI Huawei FusionSolar (`FusionSolarService` & `FusionSolarApiClient`)
- Terhubung langsung ke REST OpenAPI Huawei FusionSolar Cloud (`/thirdData/getStationRealKpi`, `/thirdData/getDevRealKpi`, `/thirdData/getDevList`).
- **Proteksi Rate-Limiting Huawei WAF (407 Anti-Blocking 2x)**:
  - Pembatasan maksimal 2 percobaan (`maxAttempts = 2`) untuk pemanggilan `/getDevList`.
  - Jika terjadi error `failCode: 407` pada percobaan ke-2, retry langsung dihentikan seketika tanpa delay buang waktu, melindungi kuota WAF gateway Huawei.
  - Abort instan pengambilan snapshot stasiun jika daftar inverter stasiun tidak dapat diperoleh.
- Mekanisme caching dan retry tangguh dengan fail-safe offline resilience: jika koneksi internet terputus, aplikasi tetap menampilkan data telemetry terakhir dengan indikator status sinkronisasi.
- Telemetry Real-time Teknis Murni (Bebas ESG):
  - **Daya Aktif (Active Power)**: kW output sesaat total dan per inverter.
  - **Yield Harian & Kumulatif**: Total produksi energi kWh per hari dan total lifetime MWh.
  - **Radiasi Matahari (Irradiance)**: Nilai insolasi W/m² dan kWh/m² dari pyranometer stasiun cuaca terintegrasi per plant (MSW & Kelanis).
  - **Performance Ratio (PR)**: Rasio performa aktual terhadap kapasitas teoritis radiasi surya (target $\ge 80\%$) per plant.
  - **Quad-Metrik Homepage**: 4 metrik operasional per plant pada baris bawah kartu Solar PV homepage: MSW Irr, MSW PR, Kelanis Irr, dan Kelanis PR.

##### C. Fitur Antarmuka Halaman `SolarDetailPage` & `InverterDetailPage`
1. **Hero Generation Card**:
   - Total daya aktif surya saat ini (kW) dengan animasi ambient sun glow.
   - Indikator kondisi cuaca dan radiasi matahari.
2. **Dual-Plant Toggle Tab**:
   - Pilihan tab cepat antara "Semua Plant", "PLTS MSW (400 kWp)", dan "PLTS Kelanis (468 kWp)".
3. **Inverter Grid Live Monitor (`InverterDetailPage`)**:
   - Grid 12 kartu inverter dengan indikator status LED hijau (*Running*), kuning (*Warning*), merah (*Fault*), abu-abu (*Offline*).
   - Metrik daya keluaran (kW), efisiensi inverter (%), tegangan DC/AC, dan suhu operasional heatsink.
   - Bersih dari komponen ESG per inverter guna memaksimalkan visibilitas metrik kelistrikan.
   - Tap pada kartu membuka halaman detail spesifik tiap inverter.
4. **Aliran Energi & Akses Cuaca (`SolarEnergyFlowWidget`)**:
   - Diagram alir animasi transmisi daya surya ke beban/grid.
   - Tombol 1-klik di header membuka `WeatherPage` untuk prakiraan cuaca Tanjung & Kelanis.
5. **Landscape Full-Screen Visualizer (`SolarLandscapeTrendPage`)**:
   - Rotasi otomatis atau tombol ekspansi untuk melihat grafik kurva produksi radiasi & daya dalam format layar lebar (landscape) dengan interval 15 menit, 1 jam, dan harian.
6. **Numeric Data Table Modal (`SolarNumericTrendSheet`)**:
   - Lembar data tabular numerik untuk audit log sheet operasional dengan kemampuan filter tanggal dan waktu.
7. **WhatsApp Production Report Generator (`share_plus`)**:
   - Tombol 1-klik untuk men-generate pesan ringkasan produksi surya berformat rapi untuk grup koordinasi WhatsApp operasional pembangkit.
...
| Solar PV dedicated page & FusionSolar OpenAPI (Dual-plant 868 kWp) | ✅ Sudah (v4.4) | P1 |
| Plant Overview Detail Page & Diagram Alir Busbar 20kV / 70kV | ✅ Sudah (v4.5) | P1 |
| Warehouse Multi-Item & D365 ERP Integration | ✅ Sudah (v4.3) | P1 |
...
*PRD v4.3 (30 Aug 2026) — Integrasi penuh modul Warehouse & Pengambilan Material Multi-Item Microsoft Dynamics 365 (D365 SCM), scanner barcode/QR kamera, dan master data dimensi.*
*PRD v4.4 (Sep 2026) — Overhaul Penuh Modul Solar PV: Dual-Plant 868 kWp (MSW 400 kWp + Kelanis 468 kWp), integrasi backend OpenAPI Huawei FusionSolar, SolarDetailPage, Inverter Grid live monitor, landscape visualizer, dan report WhatsApp.*
*PRD v4.5 (Sep 2026) — Implementasi Penuh Plant Overview Detail Page & Integrasi Firebase RTDB `excel_data/overview` (14 parameter umum), Diagram Alir Energi SCADA Animasi (Busbar 20kV / 70kV), Evaluasi Kondisi Operasional Unit 1 & 2, CEMS 4 Parameter Utama, dan Trend Chart Interaktif.*
*Disiapkan oleh: M. Farhan Tandia (IC&IT Supervisor, MSW)*
*Tanggal: September 2026 · Status: FINAL — Development & Integration Verified*
