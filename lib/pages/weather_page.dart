...
class _WeatherPageState extends State<WeatherPage> with SingleTickerProviderStateMixin {
                      ...
                      child: SingleChildScrollView(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            // 6. Plant Operational Advisory
                            _buildOperationalAdvisories(isNight),

                            const SizedBox(height: 14),

                            // 7. 5-Day Synoptic Weather Outlook
  ...
  // ===========================================================================
  // 6. PLANT OPERATIONAL ADVISORY (JETTY, STOCKPILE, INVERTER SHELTER)
  // ===========================================================================
  Widget _buildOperationalAdvisories(bool isNight) {
    final temp = (_currentWeather?['main']?['temp'] as num?)?.toDouble() ?? 30.0;
    final humidity = (_currentWeather?['main']?['humidity'] as num?)?.toInt() ?? 70;
    final wind = (_currentWeather?['wind']?['speed'] as num?)?.toDouble() ?? 2.5;
    final gust = (_currentWeather?['wind']?['gust'] as num?)?.toDouble();
    final rain = (_currentWeather?['rain']?['1h'] as num?)?.toDouble() ?? 0.0;

    final thermalRisk = WeatherService.calculateThermalDeratingRisk(tempC: temp, humidityPct: humidity);
    final windRisk = WeatherService.calculateJettyWindRisk(windSpeedMs: wind, gustMs: gust);
    final moistureRisk = WeatherService.calculateStockpileMoistureRisk(humidityPct: humidity, rainMm: rain);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(Icons.shield_outlined, size: 16, color: isNight ? const Color(0xFF38BDF8) : const Color(0xFFFFD54F)),
              const SizedBox(width: 8),
              const Expanded(
                child: FittedBox(
                  fit: BoxFit.scaleDown,
          ...
          _buildAdvisoryCard(
            isNight: isNight,
            title: 'Inverter Shelter & Thermal Risk',
            subtitle: thermalRisk['title'] as String,
            detail: thermalRisk['detail'] as String,
            level: thermalRisk['level'] as PlantRiskLevel,
            icon: Icons.solar_power_rounded,
          ),
          const SizedBox(height: 8),
          _buildAdvisoryCard(
            isNight: isNight,
            title: 'Terminal Kelanis Jetty & Tongkang',
            subtitle: windRisk['title'] as String,
            detail: windRisk['detail'] as String,
            level: windRisk['level'] as PlantRiskLevel,
            icon: Icons.directions_boat_rounded,
          ),
          const SizedBox(height: 8),
          _buildAdvisoryCard(
            isNight: isNight,
            title: 'Stockpile Batubara (Moisture)',
            subtitle: moistureRisk['title'] as String,
            detail: moistureRisk['detail'] as String,
            level: moistureRisk['level'] as PlantRiskLevel,
      ...
      child: Row(
        ...
        children: [
          Container(
            ...
            child: Icon(icon, size: 20, color: level.color),
          ),
