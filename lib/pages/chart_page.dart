...
class _ChartPageState extends State<ChartPage> {
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 1; i < widget.fullData.length; i++) {
      ...
      DateTime? dt;
      try {
        final str = row[0].toString();
        dt = DateTime.tryParse(str) ?? DateTime.tryParse(str.replaceAll(' ', 'T'));
      } catch (_) {}

      if (dt == null) {
        int dtIdx = widget.header.indexWhere((h) => h.trim().toUpperCase() == "DATETIME");
        if (dtIdx != -1 && dtIdx < row.length) {
          final s = row[dtIdx].toString();
          dt = DateTime.tryParse(s) ?? DateTime.tryParse(s.replaceAll(' ', 'T'));
        }
      }
      ...
      if (yValue != null) {
        dt ??= DateTime.now().subtract(Duration(minutes: (widget.fullData.length - i) * 5));
                          ...
                          sideTitles: SideTitles(
                            ...
                            getTitlesWidget: (value, meta) => SideTitleWidget(
                              ...
                              child: Text(
                                ...
                                style: const TextStyle(
                                    fontSize: AppTheme.fs12, color: Colors.grey),
                              ),
                            ),
                          ),
                        ),
