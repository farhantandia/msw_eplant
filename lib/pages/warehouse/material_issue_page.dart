...
class _MaterialIssuePageState extends State<MaterialIssuePage> {
            ...
            SnackBar(
              ...
              content: Row(
                children: [
                  ...
                  Text('D365 Login Successful: ${session.displayName}'),
                ],
              ),
            ),
          );
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          ...
          content: Text('You have logged out from D365 account'),
        ),
      );
    }
  }
  ...
  void _initD365MasterData() {
    ...
    _employeeOptions = [...D365Service.getEmployeeList(), '+ Manual Input / Other Code...'];
  ...
  void _removeItem(int index) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        backgroundColor: AppColors.surface,
        content: Text('Item ${removed.itemName} removed from list'),
  ...
  void _editItemQuantity(int index) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text(
          'Edit Quantity (${item.unitType})',
          style: const TextStyle(color: AppColors.text, fontSize: AppTheme.fs16, fontWeight: FontWeight.bold),
        ),
        content: Column(
          ...
          children: [
            Text(
              item.itemName,
              style: const TextStyle(color: AppColors.textSub, fontSize: AppTheme.fs13, fontWeight: FontWeight.w600),
            ),
            const SizedBox(height: 8),
            Text(
              'Max D365 stock: ${item.availableStock} ${item.unitType}',
              style: const TextStyle(color: AppColors.general, fontSize: AppTheme.fs12, fontWeight: FontWeight.w500),
            ),
            const SizedBox(height: 16),
            TextField(
              controller: qtyController,
              keyboardType: const TextInputType.numberWithOptions(decimal: true),
              style: const TextStyle(color: Colors.white, fontSize: AppTheme.fs16, fontWeight: FontWeight.bold),
                ...
                if (newQty > item.availableStock) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      ...
                      content: Text('Quantity exceeds available stock (${item.availableStock} ${item.unitType})'),
                    ),
                  );
                  return;
                }
  ...
  Future<void> _submitToD365() async {
    ...
    if (_currentD365User == null) {
      final shouldLogin = await showDialog<bool>(
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: const Text(
            'To register material issue in D365, please log in to your Microsoft Dynamics 365 account first to get official user credentials.',
            style: TextStyle(color: AppColors.textSub, fontSize: AppTheme.fs13),
          ),
          actions: [
            TextButton(
    ...
    if (_woController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            backgroundColor: AppColors.danger,
            content: Text('Work Order (WO) number is required!')),
      );
      return;
    }
    ...
    if (effectiveEmployee.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            backgroundColor: AppColors.danger,
            content: Text('Request By / Employee is required!')),
      );
      return;
    }
    ...
    if (_items.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          backgroundColor: AppColors.danger,
          content: Text('No items selected! Add at least 1 item.'),
        ),
      );
      return;
    }
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Row(
          children: [
            ...
            Text(
              ...
              style: TextStyle(color: AppColors.text, fontSize: AppTheme.fs16, fontWeight: FontWeight.bold),
            ),
          ],
        ),
        content: Column(
          ...
          children: [
            Text(
              'Material issue data will be registered to Microsoft Dynamics 365.',
              style: const TextStyle(color: AppColors.textSub, fontSize: AppTheme.fs13),
            ),
            ...
            Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  _buildReviewItem('WO No', _woController.text.trim()),
                  _buildReviewItem('Request By', effectiveEmployee),
                  _buildReviewItem('D365 Employee Code', D365Service.extractEmployeeCode(effectiveEmployee)),
                  _buildReviewItem('Warehouse Location', _selectedWarehouse),
                  ...
                  _buildReviewItem('Item Count', '${_items.length} Items'),
                  _buildReviewItem('Total Quantity', '${_items.fold(0.0, (sum, it) => sum + it.quantity)} Units'),
                ],
              ),
            ),
          ],
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () => Navigator.pop(ctx, true),
            child: const Text('Yes, Submit to D365', style: TextStyle(fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    );
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(backgroundColor: AppColors.danger, content: Text('Failed to submit to D365: $e')));
      }
    }
  }
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Column(
          children: [
            ...
            Text(
              'Issue Successful!',
              style: TextStyle(color: AppColors.text, fontSize: AppTheme.fs18, fontWeight: FontWeight.bold),
        ...
        content: Column(
          ...
          children: [
            const Center(
              child: Text(
                'Material issue transaction recorded in D365',
                style: TextStyle(color: AppColors.textSub, fontSize: AppTheme.fs13),
                textAlign: TextAlign.center,
              ),
            ),
            ...
            Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  _buildReviewItem('D365 Journal No', result.d365JournalNo ?? '-'),
                  _buildReviewItem('Work Order No', result.woNumber),
                  _buildReviewItem('Request By', result.submittedBy),
                  _buildReviewItem('D365 Employee Code', result.submittedByCode),
                  _buildReviewItem('Status', result.status),
                  _buildReviewItem('Total Items', '${result.items.length} Material Types'),
                  _buildReviewItem('Transaction Time', DateFormat('dd MMM yyyy, HH:mm').format(result.transactionDate)),
                ],
              ),
            ),
          ],
        ...
        actions: [
          ElevatedButton(
            ...
            child: const Text('Back to Warehouse', style: TextStyle(fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    );
  ...
  Widget _buildReviewItem(String label, String value) {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          Text(label, style: const TextStyle(color: AppColors.textSub, fontSize: AppTheme.fs12)),
          Flexible(
            child: Text(
              value,
              textAlign: TextAlign.end,
              style: const TextStyle(color: AppColors.text, fontSize: AppTheme.fs12, fontWeight: FontWeight.bold),
            ),
          ),
        ],
      ),
                      ...
                      children: [
                          ...
                          children: [
                            Expanded(
                              child: _buildInputField(
                                label: 'Work Order Number (WO No) *',
                                controller: _woController,
                                hint: 'Enter WO No or select from D365',
                            ...
                            Padding(
                              ...
                              child: ElevatedButton.icon(
                                ...
                                label: const Text(
                                  'WO List',
                                  style: TextStyle(fontSize: AppTheme.fs12, fontWeight: FontWeight.bold),
                                ),
                              ),
                            ),
                          ],
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Text(
                                        ...
                                        style: const TextStyle(color: AppColors.textSub, fontSize: AppTheme.fs11),
                                      ),
                                      const SizedBox(height: 2),
                                      Row(
                                        children: [
                                          ...
                                          const Text(
                                            ...
                                            style: TextStyle(color: AppColors.textDim, fontSize: AppTheme.fs11),
                                          ),
                                        ],
                                      ),
                                    ],
                        ...
                        if (!_isManualEmployee) ...[
                          _buildDropdownField(
                            ...
                            onChanged: (val) {
                              if (val != null) {
                                if (val == '+ Manual Input / Other Code...') {
                                  setState(() {
                          ...
                          Column(
                            ...
                            children: [
                              ...
                              TextField(
                                ...
                                style: const TextStyle(color: AppColors.text, fontSize: AppTheme.fs13),
                                decoration: InputDecoration(
                                  hintText: 'e.g. 61000003 or 61000003 - Name',
                                  hintStyle: const TextStyle(color: AppColors.textDim, fontSize: AppTheme.fs12),
                        ...
                        Container(
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              Text(
                                'D365 Code sent: ${D365Service.extractEmployeeCode(_isManualEmployee
                        ...
                        _buildInputField(
                          ...
                          hint: 'Material issue remarks or notes...',
                    ...
                    child: Column(
                      ...
                      children: [
                            ...
                            child: Column(
                              children: [
                                ...
                                const Text(
                                  ...
                                  style: TextStyle(color: AppColors.textSub, fontSize: AppTheme.fs13, fontWeight: FontWeight.w500),
                                ),
                                const SizedBox(height: 4),
                                const Text(
                                  'Tap the button below for manual input or barcode scanning',
                                  style: TextStyle(color: AppColors.textDim, fontSize: AppTheme.fs11),
                                  textAlign: TextAlign.center,
                                ),
                              ],
                                        ...
                                        children: [
                                          ...
                                          Row(
                                            children: [
                                              ...
                                              const SizedBox(width: 8),
                                              Text(
                                                'Stock: ${item.availableStock} ${item.unitType}',
                                                style: const TextStyle(color: AppColors.textDim, fontSize: AppTheme.fs11),
                                              ),
                                            ],
                                          ),
                                        ],
                        ...
                        OutlinedButton.icon(
                          ...
                          label: const Text(
                            '+ Add Material Item',
                            style: TextStyle(fontSize: AppTheme.fs14, fontWeight: FontWeight.bold),
                          ),
                        ),
                      ],
                    ),
                      ...
                      child: Row(
                        ...
                        children: [
                          Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              const Text('Total Issue', style: TextStyle(color: AppColors.textSub, fontSize: AppTheme.fs11)),
                              Text(
                                '${_items.length} Items ($totalQty Units)',
                                style: const TextStyle(
                                  color: AppColors.text,
                      ...
                      child: _isSubmitting
                          ? const Row(
                              ...
                              children: [
                                ...
                                Text(
                                  'Processing to D365...',
                                  style: TextStyle(fontSize: AppTheme.fs14, fontWeight: FontWeight.bold, color: Colors.black),
                                ),
                              ],
                            )
                          : const Row(
                              ...
                              children: [
                                ...
                                Text(
                                  ...
                                  style: TextStyle(fontSize: AppTheme.fs15, fontWeight: FontWeight.bold, color: Colors.black),
                                ),
                              ],
                            ),
                    ),
                ...
                const Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 2),
                      Text(
                        'Login to Microsoft Dynamics 365 is required to obtain verified user credentials.',
                        style: TextStyle(color: AppColors.textSub, fontSize: AppTheme.fs11),
                      ),
                    ],
                  ),
                ),
                  ...
                  children: [
                    Container(
                      ...
                      child: const Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          Icon(Icons.circle, size: 8, color: AppColors.general),
                          SizedBox(width: 4),
                          Text(
                            'D365 CONNECTED',
                            style: TextStyle(
  ...
  Widget _buildInputField({
    ...
    return Column(
      ...
      children: [
        Text(
          label,
          style: const TextStyle(color: AppColors.textSub, fontSize: AppTheme.fs12, fontWeight: FontWeight.w600),
        ),
        const SizedBox(height: 6),
        TextField(
...
class _AddItemModalState extends State<_AddItemModal> {
  ...
  Future<void> _checkD365Stock() async {
    ...
    if (rawNumber.isEmpty) {
      setState(() {
        _errorMessage = 'Please enter or scan item number first!';
      });
      return;
    }
    ...
    try {
      ...
      if (item == null) {
        setState(() {
          _isChecking = false;
          _errorMessage = 'Item "$rawNumber" not found in D365 system!';
        });
      } else if (item.availableStock <= 0) {
        setState(() {
          _isChecking = false;
          _verifiedItem = item;
          _errorMessage = 'Item ${item.itemName} is out of stock (0 ${item.unitType})!';
        });
      } else {
        setState(() {
          _isChecking = false;
      ...
      setState(() {
        _isChecking = false;
        _errorMessage = 'Failed to connect to D365 API: $e';
      });
    }
  }
  ...
  void _submitItem() {
    ...
    if (qty == null || qty <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          backgroundColor: AppColors.danger,
          content: Text('Please enter a valid issue quantity (> 0)!'),
        ),
      );
      return;
    }
    ...
    if (qty > _verifiedItem!.availableStock) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          backgroundColor: AppColors.danger,
          content: Text(
            'Quantity ($qty ${_verifiedItem!.unitType}) exceeds available stock (${_verifiedItem!.availableStock} ${_verifiedItem!.unitType})!',
          ),
        ),
      );
      return;
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const Text(
                '1. Enter Item No or Scan Barcode',
                style: TextStyle(color: AppColors.textSub, fontSize: AppTheme.fs12, fontWeight: FontWeight.w600),
              ),
              const SizedBox(height: 6),
              Row(
                children: [
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: Text(
                    _isChecking ? 'Checking D365...' : 'Check D365 Stock',
                    style: const TextStyle(fontSize: AppTheme.fs12, fontWeight: FontWeight.bold),
                  ),
                ),
              ),
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      const SizedBox(width: 8),
                      Expanded(
                        child: Text(_errorMessage!, style: const TextStyle(color: AppColors.danger, fontSize: AppTheme.fs12)),
                      ),
                    ],
                  ),
                ),
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: Text(
                              'Stock: ${_verifiedItem!.availableStock} ${_verifiedItem!.unitType}',
                              style: TextStyle(
                      ...
                      Row(
                        children: [
                          ...
                          const SizedBox(width: 4),
                          Text(
                            'Unit Type: ${_verifiedItem!.unitType} (from D365)',
                            style: const TextStyle(color: AppColors.textSub, fontSize: AppTheme.fs11),
                          ),
                          const Spacer(),
                ...
                if (_verifiedItem!.availableStock > 0) ...[
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      label: Text(
                        'Add ${_verifiedItem!.itemName.split(' ')[0]} to Form',
                        style: const TextStyle(fontSize: AppTheme.fs14, fontWeight: FontWeight.bold),
                      ),
                    ),
                  ),
                ],
        ...
        children: [
          ...
          Container(
            ...
            child: const Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Displaying active Work Orders (not completed) registered in D365.',
                    style: TextStyle(color: AppColors.textSub, fontSize: AppTheme.fs11),
                  ),
                ),
              ],
          ...
          Padding(
            ...
            child: TextField(
              ...
              style: const TextStyle(color: AppColors.text, fontSize: AppTheme.fs13),
              decoration: InputDecoration(
                hintText: 'Search WO No, Title, or Equipment...',
                hintStyle: const TextStyle(color: AppColors.textDim, fontSize: AppTheme.fs12),
                      ...
                      children: [
                        ...
                        Text(
                          _searchQuery.isEmpty
                              ? 'No active Work Orders in D365'
                              : 'Work Order "$_searchQuery" not found',
                          style: const TextStyle(color: AppColors.textSub, fontSize: AppTheme.fs13),
                        ),
                      ],
                    ),
...
class _D365LoginModalState extends State<D365LoginModal>
  ...
  Future<void> _handlePresetLogin(D365UserSession preset) async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _isLoading = false;
          _errorMessage = 'Failed to sign in to D365: $e';
        });
      }
    }
  }
  ...
  Future<void> _handleManualLogin() async {
    ...
    if (code.isEmpty) {
      setState(() => _errorMessage = 'D365 Employee Code is required!');
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _isLoading = false;
          _errorMessage = 'Failed to login to D365: $e';
        });
      }
    }
  }
        ...
        children: [
                        ...
                        const Column(
                          ...
                          children: [
                            ...
                            Text(
                              ...
                              style: TextStyle(
                                  color: AppColors.textDim, fontSize: AppTheme.fs11),
                            ),
                          ],
                        ),
                      ],
          ...
          Container(
            ...
            child: TabBar(
              ...
              labelStyle:
                  const TextStyle(fontSize: AppTheme.fs12, fontWeight: FontWeight.bold),
              tabs: const [
                Tab(
                  icon: Icon(Icons.badge_outlined, size: 16),
                  text: 'Executor Accounts',
                ),
                Tab(
                  icon: Icon(Icons.person_add_alt_1_outlined, size: 16),
                  text: 'Other Account / Manual',
                ),
              ],
            ),
          ),
                ...
                children: [
                  ...
                  Expanded(
                    child: Text(
                      _errorMessage!,
                      style: const TextStyle(
                          color: AppColors.danger, fontSize: AppTheme.fs12),
                    ),
                  ),
                ],
              ),
                ...
                SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 4),
                      const Text(
                        'Use official Employee Number registered in PLTU MSW D365 system.',
                        style:
                            TextStyle(color: AppColors.textDim, fontSize: AppTheme.fs11),
                      ),
                      const SizedBox(height: 14),
                      TextField(
                        ...
                        keyboardType: TextInputType.number,
                        style: const TextStyle(
                            color: AppColors.text, fontSize: AppTheme.fs13),
                        decoration: InputDecoration(
                          labelText: 'Employee ID / D365 User Code *',
                          labelStyle: const TextStyle(
                              color: AppColors.textSub, fontSize: AppTheme.fs12),
                          hintText: 'e.g. 61000003',
                          hintStyle: const TextStyle(
                              color: AppColors.textDim, fontSize: AppTheme.fs12),
                      ...
                      TextField(
                        ...
                        style: const TextStyle(
                            color: AppColors.text, fontSize: AppTheme.fs13),
                        decoration: InputDecoration(
                          labelText: 'Full Name / Position (Optional)',
                          labelStyle: const TextStyle(
                              color: AppColors.textSub, fontSize: AppTheme.fs12),
                          hintText: 'e.g. Executor Shift A',
                          hintStyle: const TextStyle(
                              color: AppColors.textDim, fontSize: AppTheme.fs12),
                      ...
                      TextField(
                        ...
                        style: const TextStyle(
                            color: AppColors.text, fontSize: AppTheme.fs13),
                        decoration: InputDecoration(
                          labelText: 'D365 PIN / Password',
                          labelStyle: const TextStyle(
                              color: AppColors.textSub, fontSize: AppTheme.fs12),
                          hintText: 'Enter your D365 PIN',
                          hintStyle: const TextStyle(
                              color: AppColors.textDim, fontSize: AppTheme.fs12),
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton.icon(
                          ...
                          label: Text(
                            _isLoading ? 'Validating...' : 'Log in to D365 Account',
                            style: const TextStyle(
                                fontSize: AppTheme.fs13, fontWeight: FontWeight.bold),
                          ),
                        ),
                      ),
                    ],
