...
class _OkrProgressPageState extends State<OkrProgressPage> {
  ...
  Widget _buildGate() {
    return SingleChildScrollView(
      child: PasswordGate(
        title: 'Update OKR',
        subtitle: 'Enter Admin password to modify OKR data.\nChanges will reflect immediately for all users.',
        iconData: Icons.lock_outline_rounded,
        color: AppColors.maintenance,
        roleLabel: 'OKR Editor',
        roleIconData: Icons.track_changes_outlined,
        roleDescription: 'Edit structure and update progress of all OKRs',
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(kr.description, style: const TextStyle(fontSize: AppTheme.fs14, color: AppColors.text, height: 1.3)),
                ),
              ],
            ),
          ),
  ...
  Widget _buildNumericInput(KeyResult kr, Color color) {
    return Column(
      ...
      children: [
        Row(
          children: [
            const Text('Aktual', style: TextStyle(fontSize: AppTheme.fs14, color: AppColors.textSub)),
            Expanded(
              child: SliderTheme(
            ...
            SizedBox(
              ...
              child: TextField(
                ...
                style: TextStyle(fontSize: AppTheme.fs14, fontWeight: FontWeight.w800, color: color),
        ...
        TextField(
          ...
          decoration: InputDecoration(
            hintText: 'Progress notes / remarks...',
  ...
  Widget _buildBinaryInput(KeyResult kr, Color color) {
    ...
    return Column(
      ...
      children: [
            ...
            return Container(
              ...
              child: Column(
                children: [
                  Text(
                    '0',
                    style: TextStyle(fontSize: AppTheme.fs14, fontWeight: FontWeight.w900, color: AppColors.general),
                  ),
                  Text(cat, style: const TextStyle(fontSize: AppTheme.fs14, color: AppColors.textSub)),
                ],
              ),
            );
          }).toList(),
        ...
        const SizedBox(height: 6),
        const Text('Tap angka untuk update jika ada insiden', style: TextStyle(fontSize: AppTheme.fs14, color: AppColors.textSub)),
    ...
    return GestureDetector(
      onTap: () async {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('All OKR progress changes saved'), backgroundColor: AppColors.general),
        );
      },
      child: Container(
        ...
        child: const Column(
          children: [
            Row(
              ...
              children: [
                Icon(Icons.cloud_upload_outlined, size: 18, color: Colors.black),
                SizedBox(width: 6),
                Text(
                  'Save All & Update Firestore',
                  textAlign: TextAlign.center,
                  style: TextStyle(fontSize: AppTheme.fs14, fontWeight: FontWeight.w800, color: Colors.black),
                ),
              ],
            ),
            SizedBox(height: 4),
            Text(
              'Data will be saved to Firestore and updated in real-time for all app users.',
              textAlign: TextAlign.center,
              style: TextStyle(fontSize: AppTheme.fs14, color: Colors.black54),
            ),
          ],
        ),
      ),
