      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            const Text('ePlant', style: TextStyle(fontSize: AppTheme.fs14, color: AppColors.textSub)),
...
class _MainScaffoldState extends State<MainScaffold> {
  ...
  List<Widget> _buildTabPages() {
    return [
      HomePage(key: const PageStorageKey('home_tab'), role: _role, onSwitchRole: _switchRole),
      const plant.PlantPage(key: PageStorageKey('plant_tab')),
      KeyedSubtree(
        key: ValueKey('role_tab_${_role.name}'),
        child: _buildRoleMiddlePage(),
      ),
      SettingsPage(key: const PageStorageKey('settings_tab'), role: _role),
    ];
  }
  ...
  Future<void> _confirmExit() async {
    final shouldExit = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'Are you sure you want to exit MSW ePlant?',
      ...
      child: Scaffold(
        ...
        bottomNavigationBar: Container(
          ...
          child: BottomNavigationBar(
            ...
            elevation: 0,
            selectedLabelStyle: const TextStyle(fontSize: AppTheme.fs14, fontWeight: FontWeight.w600),
            unselectedLabelStyle: const TextStyle(fontSize: AppTheme.fs14),
          ),
        ),
      ),
    );
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Text(icon, style: const TextStyle(fontSize: AppTheme.fs14)),
            const SizedBox(height: 12),
            Text(label, style: const TextStyle(color: AppColors.textSub, fontSize: AppTheme.fs14)),
            const SizedBox(height: 4),
            Text('Page under development', style: const TextStyle(color: AppColors.textDim, fontSize: AppTheme.fs14)),
          ],
        ),
      ),
    );
